...
*Bidsmap_sample example. As indicated bij the solid arrowline, the set of DICOM value (suitable to uniquely identify the dicom series) are used here a key-set that maps onto the set of BIDS labels. The BIDS labels can contain special tags, i.e. <attribute name> and <<argument>>. When running the bidsmapper or bidscoiner, the <attribute name> will be replaced by the DICOM value, as indicated by the solid arrowline (so here, <ProtocolName> will be replaced by "t1_mprage_sag_p2_iso_1.0")*
