...
The central idea of the bidstrainer is that you know your own scan protocol and can therefore point out which files should go where in the BIDS. In order to do so, you have to put sample files for each of the scan modalities / series in your protocol (e.g. T1, fMRI, etc) in the right folder of a semantic folder tree, e.g. as indicated by the red arrow in the folder tree shown below. If you run bidstrainer with just the name of your bidsfolder, bidstrainer will create this semantic folder tree for you in the *code* subfolder (if it is not already there). Generally, when placing your sample files, it will be fairly straightforward to find your way in this semantic folder tree, but in doubt you should have a look at the [BIDS specification](http://bids.neuroimaging.io/bids_spec.pdf). Note that the deepest foldername in the tree denotes the BIDS suffix.

If all sample files have been put in the right location, you can (re)run the bidstrainer to create a bidsmap YAML file for your study. How this works is that the bidstrainer will read a set of (e.g. key dicom) attributes from your sample files that uniquely identify the particular scan sequence and, on the other, take the path-names of the sample files to infer the associated BIDS modality labels. In this way, a unique key-value mapping is defined that can be used as input for the bidsmapper tool (see next section). If this mapping is not unique (not likely but possible), or if you prefer to use other attributes, you can create your own heuristics yaml template define your own set and use that as an input for the bidstrainer. 
The set of attributes that is read from the sample files is defined in a template file, i.e. in the bidsmapper.yaml file, but it is an optional input so you can use your own attributes of choice if needed. 

![The red arrow depicts putting a raw data sample (left file browser) into the right location in the semantic folder tree (right file browser)](https://github.com/Donders-Institute/bidscoiner/blob/master/docs/sample_tree.png)
