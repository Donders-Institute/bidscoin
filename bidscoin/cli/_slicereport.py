...
examples:
  ...
  slicereport bids anat/*_T1w* --outputs x 0.3 x 0.4 x 0.5 x 0.6 x 0.7 LF z 0.3 z 0.4 z 0.5 z 0.6 z 0.7\n """
    ...
    parser.add_argument('-o','--outlinepattern', help="Globlike search pattern to select red outline images that are projected on top of the reported images (i.e. 'outlinepattern' must yield the same number of images as 'pattern'. Prepend `outlinedir:` if your outline images are in `outlinedir` instead of `bidsdir` (see examples below)`", metavar='PATTERN')
    parser.add_argument('-i','--outlineimage',   help='A common red-outline image that is projected on top of all images', default='', metavar='FILENAME')
    ...
    parser.add_argument('--suboptions',          help='Main options of slicer for creating the sub-reports (same as OPTIONS, see below). (default: OPTIONS)', nargs='+', metavar='OPTIONS')
    parser.add_argument('--suboutputs',          help='Output options of slicer for creating the sub-reports (same as OUTPUTS, see below). (default: "S 4 1600")', default=['S','4','1600'], nargs='+', metavar='OUTPUTS')
