...
def get_parser():
    ...
    parser.add_argument('-m','--maskpattern', help="Globlike search pattern (relative to the subject/session folder) to select the images from which the defacemask is computed, e.g. 'anat/*_part-mag_*_T2starw*'. If not given then 'pattern' is used", metavar='PATTERN')
    ...
    parser.add_argument('-o','--output',      help=f"A string that determines where the defaced images are saved. It can be the name of a BIDS datatype folder, such as 'anat', or of the derivatives folder, i.e. 'derivatives'. If output is left empty then the original images are replaced by the defaced images", metavar='DESTINATION')
    parser.add_argument('-c','--cluster',     help='Use the DRMAA library to submit the deface jobs to a high-performance compute (HPC) cluster. You can add an opaque DRMAA argument with native specifications for your HPC resource manager (NB: Use quotes and include at least one space character to prevent premature parsing -- see examples)', metavar='SPECS', nargs='?', const='-l walltime=00:30:00,mem=2gb', type=str)
    parser.add_argument('-a','--args',        help='Additional arguments (in dict/json-style) that are passed to pydeface (NB: Use quotes). See examples for usage', type=json.loads, default={}, metavar='DICT')
