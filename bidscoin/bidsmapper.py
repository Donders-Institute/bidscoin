...
if __name__ == "__main__":
    ...
    parser.add_argument('-i','--interactive', help='{0}: The sourcefolder is scanned for different kinds of scans without any user interaction. The resulting bidsmap can be edited afterwards using the bidseditor. {1}: The sourcefolder is scanned and the user is asked for help if an unknown run is encountered. When finished the bidseditor will be automatically launched. {2}: Same as {1} except that a preview of all encountered runs is shown in the background. Default: 1', type=int, choices=[0,1,2], default=1)
