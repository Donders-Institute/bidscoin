...
- **Nibabel2bids: a generic plugin for imaging data**. The nibabel2bids plugin wraps around `nibabel <https://nipy.org/nibabel>`__ to convert a wide variety of data formats into BIDS-valid nifti-files. Currently, the default template bidsmap is tailored to nifti source data only (but this can readily be extended), and the user has to provide the metadata (e.g. in the bidseditor GUI) for producing valid json sidecar files.
